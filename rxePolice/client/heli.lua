...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey(Config.HelicoCamAndCo)
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == Config.JobName and ESX.PlayerData.job.grade >= Config.GradePourCameraHelico then
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~<C>Tu ne peux pas descendre en rappel en conduisant")
